...
class mouseController(NotifierMixin, _base.Controller):
    __GetCursorPos = windll.user32.GetCursorPos
    __SetCursorPos = windll.user32.SetCursorPos

    def __init__(self, *args, **kwargs):
        super(mouseController, self).__init__(*args, **kwargs)
...
class Listener(ListenerMixin, _base.Listener):
    ...
    CLICK_BUTTONS = {
        WM_LBUTTONDOWN: (Buttonnn.left, True),
        WM_LBUTTONUP: (Buttonnn.left, False),
        WM_MBUTTONDOWN: (Buttonnn.middle, True),
        WM_MBUTTONUP: (Buttonnn.middle, False),
        WM_RBUTTONDOWN: (Buttonnn.right, True),
        WM_RBUTTONUP: (Buttonnn.right, False)}

    #: A mapping from message to X button events.
    X_BUTTONS = {
        WM_XBUTTONDOWN: {
            XBUTTON1: (Buttonnn.x1, True),
            XBUTTON2: (Buttonnn.x2, True)},
        WM_XBUTTONUP: {
            XBUTTON1: (Buttonnn.x1, False),
            XBUTTON2: (Buttonnn.x2, False)}}
